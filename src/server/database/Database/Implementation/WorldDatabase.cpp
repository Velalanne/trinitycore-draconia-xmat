...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_ID_LOCK, "SELECT id, gob_guid, char_entry, lock_id, x, y, z, orientation, map_id, duration_open FROM gameobject_lock WHERE gob_guid = ?", CONNECTION_SYNCH);
}
