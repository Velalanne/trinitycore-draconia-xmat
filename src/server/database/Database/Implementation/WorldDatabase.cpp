...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_ID_LOCK, "SELECT guid, gob_entry, char_entry, lock_id, x, y, z, orientation, map_id, price, duration_open, duration_owner, obtained FROM id_lock WHERE gob_entry = ?", CONNECTION_SYNCH);
}
